...
## Task 17.2 — Componente de tabla compartido ✅ completada
...
## Task 17.3 — UI: buscador único, total y paginación en `/casos` ✅ implementada (verificación manual pendiente hasta Task 17.5)
(El filtro genérico "Campo"/"Valor" de spec 07 se quitó de la UI de `/casos`; el link de `/reportes/estudios` ahora usa `?busqueda=<estudio>` en vez de `?campo=Estudio/Abogado&valor=...`. Los endpoints de exportación siguen aceptando `campo`/`valor` por compatibilidad, y ahora también `busqueda` y múltiples `estado=` para "Pagos pendientes".)
...
## Task 17.4 — Página `/casos/pagos-pendientes` ✅ implementada (verificación manual pendiente hasta Task 17.5)
